...
(() => {
  ...
  function matcher(str, start) {
    return str.slice(start).match(new RegExp(`.{${str.length - start >> 2}}`, 'g')).map((substring) => {
  ...
  const ParameterProcessors = {
    ...
    color: {
      ...
      transform: (input) => {
        ...
        const a = Number.parseInt(color.slice(6, 8), 16)
        return {
          hex: {
            rgb: `#${input.slice(0, 6)}`,
            rgba: `#${input}`,
    ...
    bytes: {
      ...
      deserialize: (input) => {
        ...
        for (let i = 0; i < len; i++) {
          idx = i * 2
          uint8[i] = Number.parseInt(`${input[idx]}${input[idx + 1]}`, 16)
        }
        return uint8
      },
    ...
    select: {
      ...
      deserialize: (input, def) => {
        const idx = Number.parseInt(input, 16)
  ...
  function createFxhashSdk(window2) {
